...
**Where `object_type_table` comes from is still open.** It is not a copy of any
block on the disc: of the 819 non-zero rows in a level-0 snapshot, **32 appear
verbatim in `FDAT.T` entry 1 at offset 9004** and the rest do not appear
anywhere. The rows for low type ids read as item stats — `0x4b0` = 1200 and
`0x3e8` = 1000 in the first row, which are weight-and-value shapes — so the
table is built at load time out of at least two sources, and the second one has
not been found. Until it is, the port takes the class bytes out of a snapshot
and labels them borrowed, the same arrangement the object scales and the object
